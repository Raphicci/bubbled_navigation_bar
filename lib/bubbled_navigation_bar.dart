...
class BubbledNavigationBar extends StatefulWidget {
  ...
  late final MenuPositionController controller;
  ...
  BubbledNavigationBar({
    Key? key,
    required this.items,
    MenuPositionController? controller,
    ...
    this.animationDuration,
        ...
        controller = controller ?? MenuPositionController(initPosition: 0),
        super(key: key);
...
class _BubbledNavigationBarState extends State<BubbledNavigationBar>
  ...
  List<GlobalKey> titlesKeys = [];
  List<GlobalKey> iconsKeys = [];
  List<double> titleWidths = [];
  List<double> iconsWidths = [];

  List<Rect> selectedItemsRects = [];
  ...
  void _initController() {
    widget.controller.removeListener(_positionControllerValueChanged);
    widget.controller.defaultAnimationDuration =
        widget.animationDuration ?? Duration(milliseconds: 500);
    widget.controller.defaultAnimationCurve = widget.animationCurve;

    // TODO remove vsync from here
    widget.controller.vsync = this;

    widget.controller.addListener(_positionControllerValueChanged);
    widget.controller.lastPosition = widget.initialIndex;
  }
  ...
  void _positionControllerValueChanged() {
    ...
    if (widget.controller.targetPosition != null &&
            (widget.controller.absolutePosition <=
                    widget.controller.targetPosition! &&
                widget.controller.targetPosition! -
                        widget.controller.lastPosition <
                    0) ||
        (widget.controller.absolutePosition >=
                widget.controller.targetPosition! &&
            widget.controller.targetPosition! - widget.controller.lastPosition >
                0)) {
      widget.controller.lastPosition = widget.controller.targetPosition;
    }
    setState(() {});
  }
  ...
  SelectionPainter? _buildSelectionPainter() {
    ...
    Rect lastRect = selectedItemsRects[widget.controller.lastPosition];
    Color lastColor =
        widget.items[widget.controller.lastPosition].bubbleColor ??
            widget.defaultBubbleColor;
    if (widget.controller.selectionNotGoingAnywhere) {
      return _selectionPainter(lastRect, lastColor);
    }

    Rect targetRect = selectedItemsRects[widget.controller.targetPosition!];
    Rect mergedRect = Rect.lerp(
        lastRect, targetRect, widget.controller.progressToTargetPosition)!;

    Color targetColor =
        widget.items[widget.controller.targetPosition!].bubbleColor ??
            widget.defaultBubbleColor;
    Color? mergedColor = Color.lerp(
        lastColor, targetColor, widget.controller.progressToTargetPosition);
  ...
  Widget _getMenuItem(int index, double unselectedItemWidth) {
    return _MenuItem(
      ...
      selectProgress: widget.controller.itemSelectedProgress(index).abs(),
      onPressed: () {
        widget.controller.lastPosition = widget.controller.targetPosition;
        widget.controller.targetPosition = index;
        widget.controller.animateToPosition(index);
        widget.onTap!(index);
      },
        ...
        child: AnimatedSize(
          ...
          child: Stack(
            ...
            children: <Widget>[
              ...
              Positioned(
                left: iconXPos,
                child: Opacity(opacity: selectProgress, child: item.activeIcon),
              ),
              Positioned(
...
class SelectionPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final path = Path()
      ...
      ..cubicTo(startX + controlPointsOffset!, startY - radius!,
          endX - controlPointsOffset!, startY - radius!, endX, startY - radius!)
      ..cubicTo(endX + controlPointsOffset!, startY - radius!, endX + radius!,
          startY - controlPointsOffset!, endX + radius!, startY)
      ..cubicTo(endX + radius!, startY + controlPointsOffset!,
          endX + controlPointsOffset!, startY + radius!, endX, startY + radius!)
      ..cubicTo(
